...
SIR_FullyMixedAdaptation = function(N, b, tmax, f, th) {
  ...
  xa = runif(N, 0, 1)
  ya = runif(N, 0, 1)
  b2=b/f #transmission range due to SA
  # Create a data frame with the generated coordinates
  dfa = data.frame(xa, ya)
  ...
  while (t <= tmax) {
    ...
    if ((sum(nextI) / N) > th) {
      ...
      ya <- runif(N, 0, 1)
      dfa <- data.frame(xa, ya)
      g1 <- spatgraph(dfa, "geometric", par = b2)
      p1 <- g1[]
      gr <- graph_from_adj_list(p1$edges)
    } else {
      # Update Agent Locations 
...
SIRnumercial = function(N, tmax, b) {
  ...
  for (h in seq_len(length(t) - 1)) {
    dS[h + 1] <- S[h] * (1 - (1 - beta)**(pi * b * b * rho * I[h] / N))
    S[h + 1]  <- S[h] - dS[h + 1]
    I[h + 1]  <- I[h] + dS[h + 1] - gamma * I[h]
    R[h + 1]  <- R[h] + gamma * I[h]
...
SIRDelay = function(N, t, b, l, f, th, d,beta) {
  ...
  su = c()                       # List to store susceptible values
  inf = c()                      # List to store infected values
  re = c()                       # List to store recovered values
  ...
  for (h in seq_len(length(t) - 1)) {
    dS[h + 1] <- S[h] * (1 - (1 - beta)**(pi * b * b * rho * I[h] / N))
    S[h + 1]  <- S[h] - dS[h + 1]
    I[h + 1]  <- I[h] + dS[h + 1] - I[h] * gamma
    R[h + 1]  <- R[h] + I[h] * gamma
    
    su = c(su, S[h + 1])    # Append susceptible count
    inf = c(inf, I[h + 1])  # Append infected count
    re = c(re, R[h + 1])    # Append recovered count
    
    # Skip processing if the delay condition isn't met
    if (h < d) {
      next
    }
    
    # Update density based on the threshold and social adaptation
    if (I[h - d + 1] > th) {
      rho = rho2  # Apply social adaptation
    } else {
      rho = rho1  # No social adaptation
    }
  }
...
# Example usage:
result <- SIRDelay(N = 500, t = 100, b = 0.05, l = 1,
                   f = 4, th = 0.1, d = 20, beta=0.1)
#####################################################################
# Define a function to get variation of peak infection with delay
getPeakInfectionWithDelay = function(N, t, b, l, f, th,beta) {
  peak_infections = c()  # List to store peak infection counts for each delay
  for (d in 1:30) {
    infections = SIRDelay(N, t, b, l, f, th, d, beta)  # Get infection counts with the current delay
